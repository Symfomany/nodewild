...
module.exports = function (app, passport) {
	...
	app.get('/api/blocks', isLoggedIn, function (req, res) {
	...
	app.get('/cart/:id', function (req, res) {
		...
		if (!sess.cart) {
			sess.cart = [];
		} else if (sess.cart.indexOf(id) == -1) {
			sess.cart.push(id);
		}

		req.session.save();
		req.flash('success', "L'ajout dans le panier a bien été effectué");
		res.redirect('/pages');
	...
	app.get('/clear-cart', function (req, res) {
		...
		req.flash('success', "Le Panier a bien été nettoyé");
		res.redirect('/pages');
	...
	app.get('/paypal', function (req, res) {
		...
		let number = Math.floor(Math.random() * (10000000000 + 1)) + 1;

		paypal.pay(number, 62, 'iPhone', 'EUR', true, function (err, url) {
			if (err) {
				console.log(err);
				return;
			}
	...
	app.get('/paypal-confirm', function (req, res) {
		...
		paypal.detail(req.query.token, req.query.PayerID, function (err, data, invoiceNumber, price) {
			...
			if (data.success) {
				...
				var query = connection.query('INSERT INTO orders SET ?', datas, function (error, results, fields) {
				...
				req.flash('error', "Le paiment a echoué :(");
				res.redirect('/pages');
